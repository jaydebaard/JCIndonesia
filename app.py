...
# Title
st.set_page_config(page_title="Kalkulator Biaya Subkontraktor (Rupiah)", layout="centered")
st.title("🧮 Kalkulator Biaya PM, ASD, EC, dan Subkontraktor (Rupiah)")

# Section: Cost Setting
st.header("1. Biaya Teknisi")
tech_rate = st.number_input("Biaya per Jam Teknisi (Rp)", value=265600.0, step=1000.0, format="%.0f")
...
for i in range(num_subcons):
    ...
    category = st.text_input(f"Nama Pekerjaan Subkontraktor #{i+1}", value=f"Kategori {i+1}")
    ...
    rate = st.number_input(f"Biaya per Jam - {category} (Rp)", min_value=0.0, step=1000.0, key=f"rate_{i}", format="%.0f")
